...
export function createDefaultWidgetLayout(): WidgetLayout {
  return [
    ...SYSTEM_WIDGET_IDS.map((id) => ({ id, enabled: true, sizePreset: 'medium' as const, position: { ...DEFAULT_POSITIONS[id] } })),
...
export function resolveWidgetLayout(layout: WidgetLayout): ResolvedWidgetLayoutItem[] {
  ...
  for (const id of SYSTEM_WIDGET_IDS) {
    if (!known.has(id)) resolved.push({ id, enabled: true, sizePreset: 'medium', position: { ...DEFAULT_POSITIONS[id] } });
  }
  return resolved;
}
